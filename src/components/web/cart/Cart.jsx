...
export default function Cart() {
  ...
  const{getCartContext,removeItemContext,clearCartContext,incraseQuantity,decraseQuantity} = useContext(CartContext);
   ...
   const incrase=async(productId)=>{
    const result = await incraseQuantity(productId);
    return result;
   }
   
   const decrase=async(productId)=>{
    const result = await decraseQuantity(productId);
    return result;
   }
   const clearCart=async()=>{
    ...
    const {data,isLoading} =useQuery("cart",getCart,incrase,decrase,clearCart,removeCart);
    console.log(data);
    if(isLoading){
        return<p>Loading...</p>
    }
    ...
    <div className="container">
      <div className="row">
        <div className="cart-items">
          <div className="products" id="products">
            <div className="item">
             ...
             {data?.products?(
                ...
                <div className="product-info">
                  <img className='ww' src={product.details.mainImage.secure_url}/>
                ...
                <div className="quantity">
                  <button onClick={()=>decrase(product.details._id)}>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width={16}
                      height={17}
                  ...
                  <button onClick={()=>incrase(product.details._id)}>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width={16}
                      height={17}
          ...
          <div className="cart-summary mx-3 mt-5">
            <h2>Cart summary</h2>
        ...
        <div>
          <button className="clear  text-white"  type="submit" onClick={()=>clearCart()}>Clear Cart</button>
